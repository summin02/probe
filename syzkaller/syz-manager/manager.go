...
func (mgr *Manager) saveCrash(crash *manager.Crash) bool {
	...
	// PROBE: Tier-based filtering.
	tier := crash.Type.Tier()
	if tier == crash_pkg.TierLow && !crash.Suppressed {
		// Tier 3: statistics only — no logs, no reports, no repro.
...
func (mgr *Manager) saveVariantProgram(crash *manager.Crash) {
	...
	if crash.Executor != nil {
		for _, entry := range entries {
			if entry.ID == crash.Executor.ExecID {
				triggerProg = entry.P
				break
			}
		}
	}
	// Fallback: use the last program in the log.
	if triggerProg == nil {
		triggerProg = entries[len(entries)-1].P
	}
	progData := triggerProg.Serialize()
	if newVariant, err := mgr.crashStore.SaveVariant(crash.Title, crash.Type, progData); err != nil {
		log.Logf(0, "failed to save variant: %v", err)
	} else if newVariant {
