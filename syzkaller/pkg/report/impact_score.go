...
// TitleToTier converts a crash title to a PROBE severity tier.
// Returns crash.TierCritical (1), crash.TierImportant (2), or crash.TierLow (3).
func TitleToTier(title string) int {
	return crash.TitleToType(title).Tier()
}

// TitlesToMinTier returns the most severe (lowest number) tier across all titles.
func TitlesToMinTier(title string, otherTitles ...string) int {
	minTier := crash.TierLow
	for _, t := range append([]string{title}, otherTitles...) {
		if tier := TitleToTier(t); tier < minTier {
			minTier = tier
		}
	}
	return minTier
